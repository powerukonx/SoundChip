      ...
      (psoundengine->pbRenderingEnable != NULL)*/)
  {
    ...
    if(mmresult == MMSYSERR_NOERROR)
    {
      ...
      else
      {
       ...
       if (TRUE == g_clVgmContext.bInit() )
       {
          if (TRUE == g_clVgmContext.bLoad (TEXT("./vgm/SonicTheHedgehog/Sonic The Hedgehog - 03 - Green Hill Zone.vgm")))
        //   if (TRUE == g_clVgmContext.bLoad (TEXT("../vgm/Alex Kidd In Miracle World/Alex Kidd in Miracle World - 03 - Main Theme.vgm")))
          // if (TRUE == g_clVgmContext.bLoad (TEXT("./vgm/Master of Darkness/Thames River.vgm")))
          // if (TRUE == g_clVgmContext.bLoad (TEXT("./vgm/SpaceHarrier/Space Harrier - 03 - Main BGM.vgm")))
           {
                // No errors
                l_bReturn = TRUE;
           }
            ...
            if (g_byBufferIdxWrite != g_byBufferIdxRead)
            {
              ...
              // if ((*g_sSoundengineex.soundengine.pbRenderingEnable) == TRUE)
              {
                ...
                for (wBufferIndex = 0; wBufferIndex < WAVEOUT_BUFFER_SIZE; wBufferIndex += 2)
                {
                 ...
                 // do
                  {
                    g_clVgmContext.bUpdate (&l_u16AudioOutput);

                    /* Get AY datas. */
                    //g_sSoundengineex.soundengine.b_Render->bUpdate (&g_sYMUpdate);
                ...
                //  while (dResamplingCounter < dResamplingTrigger);
                  ...
                  //dResamplingTrigger += (double) g_sVgmFileContext.sFileHeader.dwSN76489clock *2/ (double) g_sSoundengineex.soundengine.;
                 // dResamplingTrigger += (double) g_sVgmFileContext.sFileHeader.dwSN76489clock *2/ (double) g_sSoundengineex.soundengine.;
#if 1
                  ...
                  if (TRUE == g_bSoundEngineLeftChannelStateOn)
                  {
                    ...
                    {
                      g_abyWaveOutput[g_byBufferIdxWrite][wBufferIndex] = fFilter (l_u16AudioOutput >> 8);
                    }
                    else
                    {
                      g_abyWaveOutput[g_byBufferIdxWrite][wBufferIndex] = (l_u16AudioOutput >> 8);
                    }
                  }
                  else
                  {
                  ...
                  if (TRUE == g_bSoundEngineRightChannelStateOn)
                  {
                    ...
                    if (TRUE == g_bSoundEngineFilterStateOn)
                    {
                      g_abyWaveOutput[g_byBufferIdxWrite][wBufferIndex+1] = fFilter(l_u16AudioOutput >> 8);
                    }
                    else
                    {
                      g_abyWaveOutput[g_byBufferIdxWrite][wBufferIndex+1] = (l_u16AudioOutput >> 8);
                    }
                  }
                  else
                  {
                  ...
                  if (TRUE == g_bSoundEngineLeftChannelStateOn)
                  {
                    ...
                    if (TRUE == g_bSoundEngineFilterStateOn)
                    {
                      g_abyWaveOutput[g_byBufferIdxWrite][wBufferIndex] = fFilter (((g_sYMUpdate.u16AnalogChannelB_Out + (g_sYMUpdate.u16AnalogChannelA_Out<<1)) >> 8) / 3);
                    }
                    else
                    {
                      g_abyWaveOutput[g_byBufferIdxWrite][wBufferIndex] = ((g_sYMUpdate.u16AnalogChannelB_Out + (g_sYMUpdate.u16AnalogChannelA_Out<<1)) >> 8) / 3;
                    }
                  }
                  else
                  {
                  ...
                  if (TRUE == g_bSoundEngineRightChannelStateOn)
                  {
                    ...
                    if (TRUE == g_bSoundEngineFilterStateOn)
                    {
                      g_abyWaveOutput[g_byBufferIdxWrite][wBufferIndex+1] = fFilter(((g_sYMUpdate.u16AnalogChannelB_Out + (g_sYMUpdate.u16AnalogChannelC_Out<<1)) >> 8) / 3);
                    }
                    else
                    {
                      g_abyWaveOutput[g_byBufferIdxWrite][wBufferIndex+1] = ((g_sYMUpdate.u16AnalogChannelB_Out + (g_sYMUpdate.u16AnalogChannelC_Out<<1)) >> 8) / 3;
                    }
                  }
                  else
                  {
